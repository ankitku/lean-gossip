...
For the dominance plot, we fix the churn level at $c = 0.2$, the
empirically grounded value from Kiffer~\etal, and extract metrics
directly for each $(D, \Dlazy)$ configuration using
Algorithm~\ref{alg:aggregate}.

\begin{algorithm}[H]
\caption{Filter Results at Fixed Churn Level ($c = 0.2$)}
\label{alg:aggregate}
\begin{algorithmic}[1]
...
\ForAll{$r \in \mathcal{R}$}
    \If{$r.c = 0.2$}
        \State $\mathcal{A} \gets \mathcal{A} \cup \{(r.D,\; r.\Dlazy,\; r.\bar{\delta},\; r.\beta_{\mathit{norm}},\; r.\mathit{p99})\}$
    \EndIf
\EndFor
...
Latency is retained in $\mathcal{A}$ for visualization but
...
\label{alg:pareto}
\begin{algorithmic}[1]
\Require Filtered results $\mathcal{A} = \{(D, \Dlazy, \bar{\delta}, \beta_{\mathit{norm}}, \mathit{p99})\}$ at $c=0.2$
...
\Function{Dominates}{$A$, $B$} \Comment{Does $A$ dominate $B$ on delivery rate and cost?}
    \State \Return $(\bar{\delta}_A \geq \bar{\delta}_B) \land ({\beta_{\mathit{norm},A}} \leq {\beta_{\mathit{norm},B}})$
    \Statex \hspace{\algorithmicindent} $\land \; \bigl((\bar{\delta}_A > \bar{\delta}_B) \lor ({\beta_{\mathit{norm},A}} < {\beta_{\mathit{norm},B}})\bigr)$
\EndFunction
